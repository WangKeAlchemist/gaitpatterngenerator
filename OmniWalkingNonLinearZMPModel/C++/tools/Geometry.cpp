...
/*! Overloaded version of the C++ output operator for
    Vectors. This operator makes it possible to use Vectors
    in output statements (e.g.  cout << v). The x- and y-coordinates
    of the Vector are printed in the format (x,y).

    \param os output stream to which information should be written
    \param v a Vector which must be printed
...
/*! This method writes the current Vector to standard output. It
    can also print a polar representation of the current Vector.

    \param cs a CoordSystemtT indicating whether a POLAR or CARTESIAN
     representation of the current Vector should be printed */
void Vector::show( CoordSystemT cs )
{
  if( cs == CARTESIAN )
    cout << *this << endl;
  else
    cout << "( r: " << length( ) << ", phi: " << getDirection( ) << "  )";
}

/*! This method writes the current Vector to a string. It can
    also write a polar representation of the current Vector.

    \param cs a CoordSystemtT indicating whether a POLAR or CARTESIAN
     representation of the current Vector should be written
    \return a string containing a polar or Cartesian representation of the
    current Vector depending on the value of the boolean argument */
string Vector::str( CoordSystemT cs )
{
    char buf[ 1024 ];
    ...
    if( cs == CARTESIAN )
        ...
        sprintf( buf, "( r: %f, phi: %f )", length( ), getDirection( ) );
...
{
    return Line(m_b,-m_a,m_a*pos.getY() - m_b*pos.getX());
}
...
{
  Line l(*this);
  Vector close_pt = l.getPointOnLineClosestTo(pt);
  //cout << "pt " << pt << "close_pt " << close_pt << "\n";
  //ORIG if (OnRay(close_pt))
  if (InRightDir(close_pt)) {
    return close_pt;
  }
  else {
...
{
    cout << "closest " << GetClosestPoint(pt)
         << "dist " << pt.getDistanceTo(GetClosestPoint(pt)) << "\n";
